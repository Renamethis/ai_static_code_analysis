...
def plot_training_graph(train_losses, eval_losses, bleu_scores):
...
def read_examples(filename):
    ...
    return examples
def predict_samples(model, dataloader, tokenizer, args):
    model.eval()
    device = args.device
    predictions = []
    ...
    with torch.no_grad():
        for batch in dataloader:
            ...
            # Ensure the tensors have the correct shape before passing to the model
            source_mask = source_mask.unsqueeze(-1) if source_mask.dim() == 1 else source_mask
            att_mask = att_mask.unsqueeze(1) if att_mask.dim() == 1 else att_mask
            position_idx = position_idx.unsqueeze(-1) if position_idx.dim() == 1 else position_idx

            # Forward pass through the model
            preds = model(source_ids, source_mask, position_idx, att_mask)
            
            for pred in preds:
                pred_text = tokenizer.decode(pred[0].cpu().numpy(), skip_special_tokens=True)
                predictions.append(pred_text)
    
    model.train()
    return predictions

def display_validation_samples(model, dataloader, tokenizer, args):
    predictions = predict_samples(model, dataloader, tokenizer, args)
    
    print("\nValidation Samples:")
    for i, (pred, example) in enumerate(zip(predictions[:5], dataloader.dataset.examples[:5])):
        print(f"Buggy Code {i + 1}: {example['buggy']}")
        print(f"Fixed Code {i + 1}: {example['fixed']}")
        print(f"Model Prediction {i + 1}: {pred}\n")
...
def convert_examples_to_features(examples, tokenizer, args,stage=None):
    ...
    for example_index, example in enumerate(tqdm(examples,total=len(examples))):
        ##extract data flow
        code_tokens,dfg=extract_dataflow(example['buggy'],parsers['java'],'java')
        ...
        if stage=="test":
            ...
            target_tokens = tokenizer.tokenize(example['fixed'])[:args.max_target_length-2]
        ...
        for epoch in range(args.num_train_epochs):
            ...
            if args.do_eval and epoch in [ int(args.num_train_epochs*(i+1)//20) for i in range(20)]:
                ...
                if 'dev_loss' in dev_dataset:
                    ...
                    eval_examples = val #read_examples(args.dev_filename)
                ...
                if 'dev_bleu' in dev_dataset:
                    ...
                    eval_examples = [{'buggy': item['buggy'], 'fixed': item['fixed']} for item in val ]
                ...
                if dev_bleu+xmatch>best_bleu:
                    ...
                    output_model_file = os.path.join(output_dir, "pytorch_model.bin")
                    torch.save(model_to_save.state_dict(), output_model_file)
    # plot_training_graph(losses, eval_losses, bleu_scores)
    with open("output.txt", "w") as output_file:
    	dump({"train": losses, "eval": eval_losses, "bleu": bleu_scores}, output_file)
    # Call the function after training
    # display_validation_samples(model, eval_data, tokenizer, args)
    if args.do_test:
        files=[]
